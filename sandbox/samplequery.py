...
def getAdjClosePrices(tickers, startdate, enddate):
    ...
    db = 'C:/DEV/GitHub/assetjet/sandbox/assetjet.db'
    ...
    pricesRaw.Date = pd.to_datetime(pricesRaw.Date) # pandas works with datetime, not date
...
def getPricesRebased(prices, startdates, base=100, frequency=None):
    """ Returns a pandas dataframe with prices rebased to base, optionally 
...
if __name__ == '__main__':
    ...
    tickersSample = ['MMM', 'ACE', 'ABT', 'ANF', 'ACN', 'ADBE', 'ADT', 'AMD', 'AES', 'AET', 'AFL', 'A', 'GAS', 'APD', 'ARG', 'AKAM', 'AA', 'ALXN', 'ATI', 'AGN', 'ALL', 'ALTR', 'MO', 'AMZN', 'AEE', 'AEP', 'AXP', 'AIG', 'AMT', 'AMP', 'ABC', 'AMGN', 'APH', 'APC', 'ADI', 'AON', 'APA', 'AIV', 'APOL', 'AAPL', 'AMAT', 'ADM', 'AIZ', 'T', 'ADSK', 'ADP', 'AN', 'AZO', 'AVB', 'AVY']
    tickersSmallSample = ['MMM', 'ACE', 'ABT', 'ANF', 'ACN', 'ADBE', 'ADT', 'AMD', 'AES', 'AET']
    startdate = '2007-10-27'
    enddate = date.today()
    
    # Get rebased prices
    closePrices, seriesbegin = getAdjClosePrices(tickersSmallSample, startdate, enddate)
    pricesRebased = getPricesRebased(closePrices, seriesbegin, base=1000, frequency='M')
